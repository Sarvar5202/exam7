...
const TEACHER_MENU = [
  { icon: <HomeRoundedIcon />,           label_uz: "Bosh sahifa",    label_ru: "Главная",           path: "/teacher/dashboard" },
  { icon: <GroupRoundedIcon />,          label_uz: "Guruhlarim",     label_ru: "Мои группы",        path: "/teacher/groups" },
  { icon: <AssignmentRoundedIcon />,     label_uz: "Uyga vazifalar",  label_ru: "Домашние задания",  path: "/teacher/homework" },
  { icon: <CalendarMonthRoundedIcon />,  label_uz: "Davomat",        label_ru: "Посещаемость",      path: "/teacher/attendance" },
];
...
export default function TeacherSidebar({ isCollapsed, toggleSidebar }) {
  ...
  // Teal accent color for Teacher dashboard to differentiate it nicely
  const activeBg = dark ? 'rgba(20,184,166,0.15)' : 'rgba(20,184,166,0.08)';
  const activeText = '#14b8a6';
  const hoverBg  = dark ? 'rgba(20,184,166,0.08)' : 'rgba(20,184,166,0.04)';
  const hoverText = '#14b8a6';
      ...
      <div style={{
        ...
        {!isCollapsed && (
          <span style={{ fontSize: '1.2rem', fontWeight: 800, letterSpacing: '-0.4px', fontFamily: '"Outfit", sans-serif' }}>
            ...
            <span style={{ color: '#14b8a6' }}>Teacher</span>
          </span>
        )}
        <button onClick={toggleSidebar} style={{
          position: 'absolute', right: -13, top: '50%', transform: 'translateY(-50%)',
          width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center',
          background: '#14b8a6', color: '#fff', border: `2px solid ${bg}`,
