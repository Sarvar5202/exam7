...
export default function AddTeacherModal({ isOpen, onClose, onAdd, items = [], initialSelected = [] }) {
  ...
  const filtered = items.filter(t => t.full_name?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className={`fixed inset-0 bg-black/50 flex items-center justify-center z-[10000] transition-opacity duration-300 ${!isOpen ? 'opacity-0' : 'opacity-100'}`} onClick={onClose}>
      <div className={`bg-white rounded-2xl w-[480px] max-w-[95vw] shadow-2xl overflow-hidden transition-all duration-300 ${!isOpen ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`} onClick={e => e.stopPropagation()}>
        <div className="flex items-start justify-between px-6 py-5 border-b border-slate-100">
          <div>
            <h2 className="text-base font-bold text-slate-900">O'qituvchi qo'shish</h2>
            <p className="text-sm text-slate-500 mt-0.5">Bitta yoki bir nechta o'qituvchini tanlang</p>
          </div>
        ...
        <div className="px-6 py-4 flex flex-col gap-3">
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="O'qituvchi qidirish..." className="w-full h-9 px-3 border border-slate-200 rounded-lg text-sm placeholder:text-slate-400 focus:border-[#6c35de] outline-none" />
          <div className="flex flex-col gap-1 max-h-[280px] overflow-y-auto">
            {filtered.map(t => (
              <label key={t.id} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                <input type="checkbox" checked={selectedIds.includes(t.id)} onChange={() => setSelectedIds(prev => prev.includes(t.id) ? prev.filter(x => x !== t.id) : [...prev, t.id])} className="w-4 h-4 accent-[#6c35de]" />
                <span className="text-sm text-slate-800">{t.full_name}</span>
              </label>
            ))}
            {filtered.length === 0 && <p className="text-center text-sm text-slate-400 py-4">O'qituvchi topilmadi</p>}
          </div>
        </div>
